import { FOOTBALL_DATA_LEAGUES, UCL_COMPETITION_CODE, canonicalTeamKey } from "../football-ai/constants.js";
import { normalizeUclStage, uclFormatEra } from "../football-ai/openfootball.js";
import { canonicalUclTeam } from "../football-ai/ucl-teams.js";

const COMPETITION_NAMES = {
  E0: ["premier league", "english premier league"],
  E1: ["championship", "efl championship", "english championship"],
  SP1: ["la liga", "laliga", "laliga ea sports", "primera division"],
  I1: ["serie a"],
  D1: ["bundesliga"],
  F1: ["ligue 1"],
  B1: ["belgian pro league", "first division a", "jupiler pro league", "jupiler league"],
  SC0: ["scottish premiership", "premiership"],
  CL: ["uefa champions league", "champions league", "uefa champions league men"],
};
...
export const THESTATSAPI_COMPETITION_IDS = {
  E0: "comp_3039",
  E1: "comp_8321",
  SP1: "comp_8814",
  I1: "comp_5840",
...
const TEAM_NAME_ALIASES = new Map([
  ...
  ["royale union st gilloise", "st gilloise"],
  ...
  // Recurring Champions League provider/archive differences
  ["afc ajax", "ajax"],
  ["inter milan", "inter"],
  ["inter milano", "inter"],
  ["psv eindhoven", "psv"],
...
export function leagueCodeForCompetition(competition) {
  ...
  const codeById = Object.keys(THESTATSAPI_COMPETITION_IDS)
    .find((code) => THESTATSAPI_COMPETITION_IDS[code] === providerId);
  if (codeById) return codeById;

  const name = normalized(competition?.name);
  const country = normalized(competition?.country);
  if (COMPETITION_NAMES.CL.includes(name)) return UCL_COMPETITION_CODE;
...
function providerAssociation(team) {
  return team?.country_code ?? team?.country?.code ?? team?.country?.name ?? team?.area?.code ?? null;
}

export function providerTeamIdentity(team, leagueCode) {
  if (leagueCode === UCL_COMPETITION_CODE) return canonicalUclTeam(team?.name, providerAssociation(team));
  const league = FOOTBALL_DATA_LEAGUES[leagueCode];
  if (!league || !team?.name) return null;
  return {
    key: canonicalTeamKey(league.countryCode, team.name),
    displayName: team.name,
    providerName: team.name,
    countryCode: league.countryCode,
    associationCode: providerAssociation(team),
  };
}
...
function providerStage(match, leagueCode) {
  if (leagueCode !== UCL_COMPETITION_CODE) return match?.stage ?? null;
  return normalizeUclStage(String(match?.stage ?? match?.round?.name ?? "").replaceAll("_", " "));
}

export function providerMatchRow(match, { leagueCode, seasonStart, aiMatchId = null } = {}) {
  const league = FOOTBALL_DATA_LEAGUES[leagueCode];
  if ((!league && leagueCode !== UCL_COMPETITION_CODE) || !match?.id || !match?.home_team?.name || !match?.away_team?.name) return null;
  const matchDate = String(match.utc_date ?? "").slice(0, 10);
  if (!/^\d{4}-\d{2}-\d{2}$/.test(matchDate)) return null;
  const homeIdentity = providerTeamIdentity(match.home_team, leagueCode);
  const awayIdentity = providerTeamIdentity(match.away_team, leagueCode);
  const stage = providerStage(match, leagueCode);
  return {
    ...
    format_era: leagueCode === UCL_COMPETITION_CODE ? uclFormatEra(seasonStart) : null,
    neutral_venue: leagueCode === UCL_COMPETITION_CODE && stage === "final",
    provider_home_team_id: String(match.home_team.id),
    provider_away_team_id: String(match.away_team.id),
    home_team_name: match.home_team.name,
    away_team_name: match.away_team.name,
    home_team_key: aiMatchId ? null : homeIdentity?.key ?? null,
    away_team_key: aiMatchId ? null : awayIdentity?.key ?? null,
    home_goals: nullableNumber(match.score?.home),
    away_goals: nullableNumber(match.score?.away),
...
function valuePair(data, category, field) {
  ...
  return {
    home: nullableNumber(pair?.home),
    away: nullableNumber(pair?.away),
  };
}
...
export function normalizeMatchStats(providerMatch, response, aiMatchId = null) {
  ...
  const corners = valuePair(data, "set_pieces", "corners");
  const fouls = valuePair(data, "discipline", "fouls");
  const yellow = valuePair(data, "discipline", "yellow_cards");
  const red = valuePair(data, "discipline", "red_cards");
  ...
  return {
    ...
    home_yellow: yellow.home,
    away_yellow: yellow.away,
    home_red: red.home,
    away_red: red.away,
...
export function enrichTrainingMatches(matches, enrichments) {
  ...
  return matches.map((match) => {
    ...
    return {
      ...
      home_big_chances: row.home_big_chances,
      away_big_chances: row.away_big_chances,
      home_box_touches: row.home_box_touches,
      away_box_touches: row.away_box_touches,
      home_final_third_entries: row.home_final_third_entries,
      away_final_third_entries: row.away_final_third_entries,
      home_possession: row.home_possession,
      away_possession: row.away_possession,
      home_corners: match.home_corners ?? row.home_corners,
      away_corners: match.away_corners ?? row.away_corners,
      home_fouls: match.home_fouls ?? row.home_fouls,
      away_fouls: match.away_fouls ?? row.away_fouls,
      home_yellow: match.home_yellow ?? row.home_yellow,
      away_yellow: match.away_yellow ?? row.away_yellow,
      home_red: match.home_red ?? row.home_red,
      away_red: match.away_red ?? row.away_red,
