...
function createTeam(key, name, initialElo) {
  return {
    ...
    history: [],
    home: { matches: 0, goalsFor: 0, goalsAgainst: 0 },
    away: { matches: 0, goalsFor: 0, goalsAgainst: 0 },
  };
}
...
export function createModelState(overrides = {}) {
  return {
    ...
    options: { ...DEFAULT_MODEL_OPTIONS, ...overrides },
    teams: {},
    leagues: {},
    calibration: null,
    trainedMatches: 0,
...
function venueStrength(team, venue, rates, priorMatches) {
  ...
  const attackRate = (record.goalsFor + priorMatches * attackBaseline) / (record.matches + priorMatches);
  const concededRate = (record.goalsAgainst + priorMatches * defenceBaseline) / (record.matches + priorMatches);
  return {
...
export function buildPrematchFeatures(state, match) {
  ...
  return {
    ...
    eloDifference: round(home.elo + state.options.homeAdvantageElo - away.elo, 2),
...
function expectedGoals(state, match) {
  ...
  const ratingDifference = home.elo + state.options.homeAdvantageElo - away.elo;
  const eloFactor = Math.exp(clamp(ratingDifference, -500, 500) * 0.00125);
  const homeRecent = recentSummary(home, state.options.formWindow);
  const awayRecent = recentSummary(away, state.options.formWindow);
  const homeFormFactor = homeRecent.matches >= 3
    ? clamp((homeRecent.goalsFor + rates.homeGoals) / (2 * rates.homeGoals), 0.72, 1.28)
    : 1;
  const awayFormFactor = awayRecent.matches >= 3
    ? clamp((awayRecent.goalsFor + rates.awayGoals) / (2 * rates.awayGoals), 0.72, 1.28)
    : 1;
...
function historyEntry(match, venue) {
  ...
  return {
    ...
    xgFor: home ? match.home_xg : match.away_xg,
    xgAgainst: home ? match.away_xg : match.home_xg,
  };
}
...
export function updateModelWithResult(state, match) {
  ...
  const away = getTeam(state, match.away_team_key, match.away_team_name);
  const league = getLeague(state, match.league_code);
  const expectedHome = 1 / (1 + 10 ** (-(home.elo + state.options.homeAdvantageElo - away.elo) / 400));
  ...
  const homeEntry = historyEntry(match, "home");
  const awayEntry = historyEntry(match, "away");
