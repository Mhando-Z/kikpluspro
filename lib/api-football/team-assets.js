...
const CLUB_TOKENS = new Set([
  ...
  "club", "kv", "ogc", "rc", "rcd", "sc", "sco", "sl", "ss", "ssc",
...
function rankedCandidates(aiTeam, apiTeams) {
  const expectedCountry = normalizedCountry(aiTeam.country_code);
  return apiTeams
...
export function matchTeamAssets(aiTeams = [], apiTeams = []) {
  ...
  for (const proposal of proposals) {
    ...
    if (assignedApiIds.has(apiId)) {
      unresolved.push({
        ...
        name: proposal.aiTeam.display_name,
        countryCode: proposal.aiTeam.country_code,
        reason: "api-team-already-linked",
        bestCandidate: proposal.apiTeam.name,
        bestScore: Number(proposal.score.toFixed(4)),
      });
      continue;
    }
  ...
  return { matches, preserved, unresolved };
}
...
export async function reconcileCachedTeamAssets(supabase) {
  ...
  try {
    ...
    await upsertBatches(supabase, "ai_teams", teamRows, "canonical_key");
    ...
    return {
      ...
      linked: teamRows.length + outcome.preserved.length,
      ...
      coverage: aiResult.data.length
        ? Number(((teamRows.length + outcome.preserved.length) / aiResult.data.length).toFixed(4))
        : 0,
      unresolved: outcome.unresolved.slice(0, 50),
      syncedAt: now,
    };
  } catch (error) {
...
export async function loadTeamAssetReport(supabase) {
  ...
  try {
    ...
    const linked = aiTeams.filter((team) => team.logo_url && team.api_football_team_id);
    const unresolved = aiTeams
      .filter((team) => !team.logo_url || !team.api_football_team_id)
      .map((team) => ({
    ...
    const byCountry = Object.values(aiTeams.reduce((summary, team) => {
      ...
      summary[key] ??= { countryCode: key, total: 0, linked: 0, unresolved: 0 };
      summary[key].total += 1;
      if (team.logo_url && team.api_football_team_id) summary[key].linked += 1;
    ...
    return {
      ...
      coverage: aiTeams.length ? Number((linked.length / aiTeams.length).toFixed(4)) : 0,
      unresolved: unresolved.slice(0, 50),
      byCountry,
      lastUpdatedAt: linked.reduce((latest, team) =>
