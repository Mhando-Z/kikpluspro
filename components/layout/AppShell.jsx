      ...
      {navigation.map((item) => {
        ...
        return (
          <Link
            ...
            onClick={onNavigate}
          >
            <Icon className="size-[1.15rem] shrink-0" strokeWidth={active ? 2.4 : 1.9} />
...
function Brand() {
  return (
    <Link className="flex items-center gap-3" href="/" aria-label="KickPulse home">
  ...
  return (
    <aside className="fixed inset-y-0 left-0 z-40 hidden w-[17.5rem] border-r border-line bg-canvas/92 p-5 backdrop-blur-2xl lg:flex lg:flex-col">
      ...
      <div className="mt-9 flex-1 overflow-y-auto pr-1">
        <p className="mb-3 px-3 text-[0.65rem] font-extrabold uppercase tracking-[0.16em] text-ink-muted">Workspace</p>
        <NavItems />
      </div>
...
function LeagueFilter() {
  ...
  const league = searchParams.get("league") ?? process.env.NEXT_PUBLIC_DEFAULT_LEAGUE_ID ?? "39";
  const season = searchParams.get("season") ?? process.env.NEXT_PUBLIC_DEFAULT_SEASON ?? "2024";
    ...
    <div className="hidden items-center gap-2 sm:flex">
      <label className="relative">
        ...
        <select
          ...
          value={league}
        >
          {leagueOptions.map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}
        </select>
      ...
      <label>
        ...
        <select
          ...
          value={season}
        >
          {[2024, 2023, 2022].map((year) => <option key={year} value={year}>{year}/{String(year + 1).slice(-2)}</option>)}
        </select>
      </label>
    </div>
  );
    ...
    <header className="sticky top-0 z-30 border-b border-line bg-canvas/82 px-4 py-3 backdrop-blur-2xl sm:px-6 lg:px-8">
      <div className="mx-auto flex max-w-[100rem] items-center justify-between gap-3">
        <div className="flex min-w-0 items-center gap-3">
          <button className="icon-button lg:hidden" onClick={onMenu} type="button" aria-label="Open menu">
    ...
    <div className="min-h-screen bg-canvas text-ink">
      ...
      <div className="lg:pl-[17.5rem]">
        ...
        <AnimatePresence mode="wait" initial={false}>
          <motion.main
            className="mx-auto min-h-[calc(100vh-4.25rem)] max-w-[100rem] px-4 py-5 sm:px-6 sm:py-7 lg:px-8 lg:py-8"
            key={routeKey}
