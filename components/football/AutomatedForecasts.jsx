...
function fixtureSignals(fixture) {
  ...
  if (prediction.confidence === "high" || Number(prediction.pick?.value) >= SIGNAL_THRESHOLDS.strongPick) {
    signals.push({ key: "strong", label: "Strong signal", tone: "border-brand/20 bg-brand/8 text-brand-strong" });
  }
  if (Number(prediction.probabilities?.over25) >= SIGNAL_THRESHOLDS.goals) {
    signals.push({ key: "goals", label: `Over 2.5 · ${percentage(prediction.probabilities.over25)}`, tone: "border-accent/20 bg-accent/8 text-accent" });
  }
  if (Number(prediction.probabilities?.bothTeamsScore) >= SIGNAL_THRESHOLDS.bothTeamsScore) {
    signals.push({ key: "btts", label: `BTTS · ${percentage(prediction.probabilities.bothTeamsScore)}`, tone: "border-warning/20 bg-warning/8 text-warning" });
  }
  return signals;
}

function matchesSignalCategory(fixture, category) {
  return category === "all" || fixtureSignals(fixture).some((signal) => signal.key === category);
}
...
function ForecastCard({ fixture, index, onOpen }) {
  ...
  return (
    ...
    >
      ...
      <div className="mt-4 flex flex-wrap gap-1.5" aria-label="Prediction signals">
        {signals.map((signal) => (
          <span
...
function SignalCategoryFilters({ activeCategory, counts, onChange }) {
  return (
    <div className="-mx-1 flex gap-2 overflow-x-auto px-1 pb-1" aria-label="Filter forecasts by signal category">
      {signalCategories.map(({ key, label, icon: Icon }) => {
        ...
        return (
          ...
          >
            ...
            {label}
            <span className={`rounded-md px-1.5 py-0.5 text-[0.6rem] tabular-nums ${active ? "bg-brand/10" : "bg-surface-soft"}`}>
              {counts[key] ?? 0}
            </span>
          </button>
        );
    ...
    <section className="surface-panel p-5 sm:p-6">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <h2 className="mt-2 text-xl font-black tracking-[-0.035em]">Signal coverage by competition</h2>
          <p className="mt-2 text-xs leading-5 text-ink-muted">
            Signal totals include the model pick plus qualifying strength, goals and both-teams-to-score indicators.
          </p>
        </div>
        {activeLeague !== "all" ? (
          <button className="button-ghost text-xs" onClick={() => onChange("all")} type="button">
            ...
            >
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  ...
                  <p className="mt-2 truncate text-sm font-black" title={league.name}>{league.name}</p>
                </div>
                <div className="text-right">
                  <p className="text-2xl font-black tabular-nums text-brand-strong">{league.signals}</p>
                  <p className="text-[0.58rem] font-extrabold uppercase tracking-[0.11em] text-ink-muted">signals</p>
                </div>
              </div>
              <div className="mt-4 grid grid-cols-3 gap-1.5 text-center">
                {[
                  ...
                  <div className="rounded-xl bg-surface px-2 py-2" key={label}>
                    ...
                    <p className="mt-0.5 text-[0.55rem] font-bold uppercase tracking-[0.08em] text-ink-muted">{label}</p>
                  </div>
                ))}
              </div>
              <div className="mt-3 flex items-center justify-between text-[0.62rem] font-bold text-ink-muted">
                <span>BTTS {league.btts}</span>
                <span>H {league.home} · D {league.draw} · A {league.away}</span>
              </div>
            </motion.button>
          );
        })}
...
export function AutomatedForecasts() {
  ...
  const categoryCounts = useMemo(() => Object.fromEntries(signalCategories.map(({ key }) => [
    key,
    key === "all" ? forecasted.length : forecasted.filter((fixture) => matchesSignalCategory(fixture, key)).length,
  ])), [forecasted]);
  ...
  const leagueStats = useMemo(() => {
    ...
    return [...leagues.values()].sort((left, right) => right.signals - left.signals || left.name.localeCompare(right.name));
  }, [forecasted]);

  const visibleForecasts = useMemo(() => {
    const needle = query.trim().toLocaleLowerCase();
    return forecasted.filter((fixture) => {
      if (activeLeague !== "all" && fixture.leagueCode !== activeLeague) return false;
  ...
  const filtersActive = Boolean(query.trim()) || activeCategory !== "all" || activeLeague !== "all";
  const resetFilters = () => {
    setQuery("");
  ...
  return (
    <div className="space-y-5">
      ...
      {forecasted.length ? (
        <LeagueSignalStats rows={leagueStats} activeLeague={activeLeague} onChange={setActiveLeague} />
      ) : null}
        ...
        <section className="surface-panel p-5 sm:p-6">
          <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-end">
            ...
            <div className="flex items-center justify-between gap-3 lg:justify-end">
              <span className="chip">
                <Filter className="size-3.5 text-brand-strong" /> Showing {visibleForecasts.length} of {forecasted.length}
              </span>
              {filtersActive ? (
                <button className="button-ghost text-xs" onClick={resetFilters} type="button">Reset</button>
              ) : null}
            </div>
          </div>
          <div className="mt-5 border-t border-line pt-4">
            <p className="mb-3 text-[0.64rem] font-extrabold uppercase tracking-[0.12em] text-ink-muted">Signal categories</p>
            <SignalCategoryFilters activeCategory={activeCategory} counts={categoryCounts} onChange={setActiveCategory} />
          </div>
        </section>
      ) : null}
      ...
      {forecasted.length ? (
        visibleForecasts.length ? (
          <motion.section className="grid gap-4 md:grid-cols-2 2xl:grid-cols-3" layout>
          ...
          <section className="surface-flat p-6 text-center sm:p-8">
            ...
            <h3 className="mt-4 text-lg font-black">No forecasts match these filters</h3>
            <p className="mx-auto mt-2 max-w-lg text-xs leading-5 text-ink-muted">Try another team name, signal category or league.</p>
