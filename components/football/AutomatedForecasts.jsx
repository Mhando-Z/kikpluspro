...
const signalCategories = [
  { key: "all", label: "All signals", icon: Layers3 },
  { key: "strong", label: "Strong picks", icon: ShieldCheck },
  { key: "home", label: "Home wins", icon: Trophy },
  { key: "draw", label: "Draws", icon: CircleDot },
  { key: "away", label: "Away wins", icon: Target },
  { key: "goals", label: "Over 2.5", icon: Goal },
  { key: "btts", label: "BTTS", icon: Gauge },
];

function fixtureSignals(fixture) {
  const prediction = fixture.prediction;
  if (!prediction) return [];
  const pickCode = prediction.pick?.code;
  const signals = [
    {
      key: pickCode === "H" ? "home" : pickCode === "D" ? "draw" : "away",
      label: prediction.pick?.label ?? "Model pick",
      tone:
        pickCode === "H"
  ...
  if (
    ...
    Number(prediction.pick?.value) >= SIGNAL_THRESHOLDS.strongPick
  ) {
    signals.push({
      key: "strong",
      label: "Strong signal",
      tone: "border-brand/20 bg-brand/8 text-brand-strong",
    });
  }
  if (Number(prediction.probabilities?.over25) >= SIGNAL_THRESHOLDS.goals) {
    signals.push({
      key: "goals",
      label: `Over 2.5 · ${percentage(prediction.probabilities.over25)}`,
      tone: "border-accent/20 bg-accent/8 text-accent",
    });
  }
  if (
    Number(prediction.probabilities?.bothTeamsScore) >=
    SIGNAL_THRESHOLDS.bothTeamsScore
  ) {
    signals.push({
      key: "btts",
      label: `BTTS · ${percentage(prediction.probabilities.bothTeamsScore)}`,
...
function matchesSignalCategory(fixture, category) {
  return (
    category === "all" ||
    fixtureSignals(fixture).some((signal) => signal.key === category)
  );
}
  ...
  const leadingKey =
    prediction.pick.code === "H"
      ...
      : prediction.pick.code === "D"
        ? "draw"
        : "awayWin";
  const signals = fixtureSignals(fixture);
  return (
    <motion.article
    ...
    >
      <div className="flex items-center justify-between gap-3">
        <div className="flex min-w-0 flex-wrap items-center gap-1.5">
          ...
          <span className="chip">
            ...
            {prediction.model?.shortFamily ?? "AI model"} v
      ...
      <div
        ...
        aria-label="Prediction signals"
      >
        {signals.map((signal) => (
          <span
            className={`rounded-full border px-2.5 py-1 text-[0.6rem] font-extrabold ${signal.tone}`}
            key={signal.key}
          >
            {signal.label}
          </span>
...
function ReportMetric({ icon: Icon, label, value, detail }) {
  return (
    <div className="rounded-2xl border border-line bg-surface-soft/55 p-4">
      <p className="flex items-center gap-2 text-[0.62rem] font-extrabold uppercase tracking-[0.11em] text-ink-muted">
        <Icon className="size-3.5" /> {label}
      </p>
      <p className="mt-2 text-lg font-black">{value}</p>
      {detail ? (
        <p className="mt-1 text-[0.65rem] text-ink-muted">{detail}</p>
      ) : null}
    </div>
  );
}
...
function PredictionReport({ fixture, model, onClose }) {
  ...
  const [odds, setOdds] = useState(() =>
    String(oddsFor(fixture, prediction.pick.code) ?? ""),
  );
  ...
  const saveBet = async (event) => {
    ...
    if (!Number.isFinite(numericStake) || numericStake <= 0)
      return toast.error("Enter a stake greater than zero.");
    if (!Number.isFinite(numericOdds) || numericOdds <= 1)
      return toast.error("Enter valid decimal odds greater than 1.00.");
    setSaving(true);
    try {
      await saveTrackedBet({
        ...
        modelPick: {
          ...
          label: prediction.pick.label,
          probability: prediction.pick.value,
        },
        selection,
        stake: numericStake,
        odds: numericOdds,
  ...
  return createPortal(
    ...
    >
      <motion.section
        aria-modal="true"
        className="surface-panel mx-auto my-3 max-w-5xl overflow-hidden"
      ...
      >
        <div className="flex items-center justify-between border-b border-line p-4 sm:px-6">
          <div>
            <p className="eyebrow">Full prediction report</p>
            <p className="mt-1 text-xs text-ink-muted">
              {fixture.leagueName}
              {fixture.competitionStage
                ? ` · ${stageLabel(fixture.competitionStage)}`
                : ""}{" "}
              · {kickoffLabel(fixture.kickoffAt)}
            </p>
          </div>
        ...
        <div className="border-b border-line bg-surface-soft/30 p-5 sm:p-7">
          <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-4 text-center">
            <div className="min-w-0">
              ...
              <h2 className="mt-2 truncate text-base font-black sm:text-xl">
                {fixture.homeTeam.name}
              </h2>
            </div>
            <div>
              <p className="text-[0.6rem] font-extrabold uppercase tracking-[0.14em] text-ink-muted">
                Model pick
              </p>
              <p className="mt-1 text-xl font-black text-brand-strong sm:text-2xl">
                {prediction.pick.label}
              </p>
              <p className="text-xs font-extrabold text-ink-muted">
                {percentage(prediction.pick.value, 1)}
              </p>
            </div>
            <div className="min-w-0">
              <span className="mx-auto block w-fit">
          ...
          <div className="mt-5 flex flex-wrap justify-center gap-2">
            <span className="chip capitalize">
              <Gauge className="size-3.5 text-accent" /> {prediction.confidence}{" "}
              confidence
            </span>
            <span className="chip">
              <ShieldCheck className="size-3.5 text-brand-strong" />{" "}
              {predictionModel.shortFamily ?? predictionModel.family} v
              {predictionModel.version}
            </span>
            {fixture.neutralVenue ? (
              <span className="chip">Neutral venue</span>
            ) : null}
          </div>
        </div>
        <div className="grid gap-6 p-5 sm:p-7 lg:grid-cols-[1.15fr_0.85fr]">
          <div>
            <p className="text-[0.65rem] font-extrabold uppercase tracking-[0.13em] text-ink-muted">
              1X2 probability
            </p>
            ...
            <div className="mt-6 grid grid-cols-2 gap-3">
              <ReportMetric
                ...
                value={`${decimal(prediction.expectedGoals.home)} – ${decimal(prediction.expectedGoals.away)}`}
              />
              <ReportMetric
                icon={BarChart3}
                label="Top scoreline"
                value={prediction.topScorelines[0]?.score ?? "—"}
                detail={percentage(prediction.topScorelines[0]?.probability, 1)}
              />
              <ReportMetric
                icon={Target}
                label="Over 2.5"
                value={percentage(prediction.probabilities.over25, 1)}
              />
              <ReportMetric
                icon={Sparkles}
                label="Both teams score"
                value={percentage(prediction.probabilities.bothTeamsScore, 1)}
              />
            </div>
          </div>
                ...
                {prediction.topScorelines.map((score, index) => (
                  ...
                  >
                    <span className="text-sm font-black">
                      <span className="mr-2 text-[0.62rem] text-ink-muted">
                        {index + 1}
                      </span>
                      {score.score}
                    </span>
                    <span className="text-xs font-extrabold text-ink-muted">
                      {percentage(score.probability, 1)}
                    </span>
                  </div>
                ))}
              </div>
            ...
            fixture.marketOdds?.away ? (
              <div className="rounded-2xl border border-line p-4">
                ...
                <div className="mt-3 grid grid-cols-3 gap-2">
                  {[
                    ["Home", fixture.marketOdds.home],
                    ["Draw", fixture.marketOdds.draw],
                    ["Away", fixture.marketOdds.away],
                  ].map(([label, value]) => (
                    <div
                      className="rounded-xl bg-surface-soft p-3 text-center"
                      key={label}
                    >
        ...
        <div className="border-t border-line p-5 sm:p-7">
          <p className="flex items-center gap-2 text-xs font-extrabold">
            <ShieldQuestion className="size-4 text-accent" /> Why the model says
            this
          </p>
          <div className="mt-3 grid gap-2 sm:grid-cols-2">
            {prediction.explanations.map((explanation) => (
              <p
                className="rounded-xl bg-surface-soft px-3 py-2.5 text-xs leading-5 text-ink-muted"
                key={explanation}
              >
                {explanation}
              </p>
        ...
        <form
          className="border-t border-line bg-surface-soft/35 p-5 sm:p-7"
          onSubmit={saveBet}
        >
          <div className="flex items-start gap-3">
            ...
            <div>
              <h3 className="font-black">Track a bet from this report</h3>
              <p className="mt-1 text-xs leading-5 text-ink-muted">
                One 1X2 selection per fixture. Saving again updates the record
          ...
          <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <label>
              <span className="mb-1.5 block text-xs font-extrabold">Note</span>
              <input
                className="form-control"
                maxLength="140"
          ...
          <div className="mt-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <p className="text-[0.65rem] leading-5 text-ink-muted">
              Tracking records your decision; it does not place a bet. Forecasts
...
function SignalCategoryFilters({ activeCategory, counts, onChange }) {
  return (
    <div
      className="-mx-1 flex gap-2 overflow-x-auto px-1 pb-1"
      aria-label="Filter forecasts by signal category"
    >
      {signalCategories.map(({ key, label, icon: Icon }) => {
        ...
        return (
          <button
            ...
            className={`flex shrink-0 items-center gap-2 rounded-xl border px-3 py-2 text-xs font-extrabold transition ${
              active
                ? "border-brand/30 bg-brand-soft text-brand-strong shadow-[0_8px_24px_rgba(12,171,120,0.10)]"
                : "border-line bg-surface text-ink-muted hover:border-brand/20 hover:text-ink"
            }`}
            key={key}
          ...
          >
            ...
            {label}
            <span
              className={`rounded-md px-1.5 py-0.5 text-[0.6rem] tabular-nums ${active ? "bg-brand/10" : "bg-surface-soft"}`}
            >
              {counts[key] ?? 0}
            </span>
          </button>
...
function LeagueSignalStats({ rows, activeLeague, onChange }) {
  return (
    <section className="surface-panel p-5 sm:p-6">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <p className="eyebrow">League signal desk</p>
          <h2 className="mt-2 text-xl font-black tracking-[-0.035em]">
            Signal coverage by competition
          </h2>
          <p className="mt-2 text-xs leading-5 text-ink-muted">
            Signal totals include the model pick plus qualifying strength, goals
            and both-teams-to-score indicators.
          </p>
        </div>
        {activeLeague !== "all" ? (
          <button
            ...
            type="button"
          >
            <X className="size-3.5" /> Clear league
          </button>
        ) : null}
      </div>
          ...
          return (
            <motion.button
              ...
              className={`rounded-2xl border p-4 text-left transition ${
                active
                  ? "border-brand/35 bg-brand-soft shadow-[0_12px_32px_rgba(12,171,120,0.10)]"
                  : "border-line bg-surface-soft/45 hover:border-brand/25 hover:bg-surface-soft"
              }`}
              key={league.code}
              layout
              onClick={() => onChange(active ? "all" : league.code)}
              type="button"
            >
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <span className="chip">{league.code}</span>
                  <p
                    className="mt-2 truncate text-sm font-black"
                    title={league.name}
                  >
                ...
                <div className="text-right">
                  <p className="text-2xl font-black tabular-nums text-brand-strong">
                    {league.signals}
                  </p>
                  <p className="text-[0.58rem] font-extrabold uppercase tracking-[0.11em] text-ink-muted">
                    signals
                  </p>
                </div>
              </div>
              <div className="mt-4 grid grid-cols-3 gap-1.5 text-center">
                {[
                  ...
                  <div className="rounded-xl bg-surface px-2 py-2" key={label}>
                    <p className="text-xs font-black tabular-nums">{value}</p>
                    <p className="mt-0.5 text-[0.55rem] font-bold uppercase tracking-[0.08em] text-ink-muted">
                      {label}
                    </p>
                  </div>
                ))}
              </div>
              <div className="mt-3 flex items-center justify-between text-[0.62rem] font-bold text-ink-muted">
                <span>BTTS {league.btts}</span>
                <span>
                  H {league.home} · D {league.draw} · A {league.away}
                </span>
              </div>
            </motion.button>
          );
...
export function AutomatedForecasts() {
  ...
  const categoryCounts = useMemo(
    () =>
      Object.fromEntries(
        signalCategories.map(({ key }) => [
          key,
          key === "all"
            ? forecasted.length
            : forecasted.filter((fixture) =>
                matchesSignalCategory(fixture, key),
              ).length,
        ]),
      ),
    [forecasted],
  ...
  const leagueStats = useMemo(() => {
    ...
    for (const fixture of forecasted) {
      const signals = fixtureSignals(fixture);
      const current = leagues.get(fixture.leagueCode) ?? {
        code: fixture.leagueCode,
      ...
      for (const signal of signals) {
        if (Object.hasOwn(current, signal.key)) current[signal.key] += 1;
      }
      leagues.set(fixture.leagueCode, current);
    }
    return [...leagues.values()].sort(
      (left, right) =>
        right.signals - left.signals || left.name.localeCompare(right.name),
    );
  }, [forecasted]);

  const visibleForecasts = useMemo(() => {
    const needle = query.trim().toLocaleLowerCase();
    return forecasted.filter((fixture) => {
      if (activeLeague !== "all" && fixture.leagueCode !== activeLeague)
        return false;
      if (!matchesSignalCategory(fixture, activeCategory)) return false;
  ...
  }, [activeCategory, activeLeague, forecasted, query]);

  const filtersActive =
    Boolean(query.trim()) || activeCategory !== "all" || activeLeague !== "all";
  const resetFilters = () => {
    setQuery("");
          ...
          <div>
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span className="chip">
                ...
                {state.data.models.length} active model{" "}
                {state.data.models.length === 1 ? "family" : "families"}
              </span>
            </div>
            ...
            <p className="mt-2 max-w-2xl text-sm leading-6 text-ink-muted">
              Competition-aware routing sends supported domestic fixtures to the
              domestic league model and Champions League fixtures to the UCL
              specialist.
            </p>
          </div>
          <div className="flex flex-wrap items-center gap-2">
        ...
        <section className="surface-panel p-5 sm:p-6">
          <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-end">
            <label className="block">
              <span className="mb-2 flex items-center gap-2 text-[0.64rem] font-extrabold uppercase tracking-[0.12em] text-ink-muted">
              ...
              <span className="relative block">
                ...
                <input
                  className="form-control h-11 pl-10 pr-10"
                  onChange={(event) => setQuery(event.target.value)}
                  placeholder="Search team, league or competition stage…"
                  type="search"
                  value={query}
                />
                {query ? (
                  <button
                    aria-label="Clear fixture search"
                    className="absolute right-2 top-1/2 flex size-7 -translate-y-1/2 items-center justify-center rounded-lg text-ink-muted transition hover:bg-surface-soft hover:text-ink"
                    onClick={() => setQuery("")}
                    type="button"
                  >
            ...
            <div className="flex items-center justify-between gap-3 lg:justify-end">
              <span className="chip">
                <Filter className="size-3.5 text-brand-strong" /> Showing{" "}
                {visibleForecasts.length} of {forecasted.length}
              </span>
              {filtersActive ? (
                <button
                  className="button-ghost text-xs"
          ...
          <div className="mt-5 border-t border-line pt-4">
            <p className="mb-3 text-[0.64rem] font-extrabold uppercase tracking-[0.12em] text-ink-muted">
              Signal categories
            </p>
            <SignalCategoryFilters
        ...
        visibleForecasts.length ? (
          ...
          >
            <AnimatePresence initial={false}>
              {visibleForecasts.map((fixture, index) => (
                <ForecastCard
                  fixture={fixture}
                  index={index}
                  key={fixture.id}
                  onOpen={() => setSelectedFixture(fixture)}
                />
              ))}
            </AnimatePresence>
          </motion.section>
