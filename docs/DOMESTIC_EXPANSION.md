...
| Code | Competition | Current fixture provider | Special handling |
| --- | --- | --- | --- |
| `E1` | EFL Championship | Football-Data.org (`ELC`) | Enabled |
| `B1` | Belgian Pro League | Not configured | Training remains available; current sync reports unavailable |
| `SC0` | Scottish Premiership | Not configured | Training remains available; current sync reports unavailable |

No new Supabase migration is required. Existing league fields are text columns.

## 1. Import free canonical history
...
import is idempotent. Previously archived enrichment stays linked to these
canonical rows; there are no future TheStatsAPI calls.

## 2. Train the isolated expansion model
...
Imported enrichment stays in Supabase and is read locally during training.
Current `E1` fixtures/results use Football-Data.org. `B1` and `SC0` remain
disabled until an accurate current provider mapping is configured; do not map
another country's competition merely to remove the warning. Normal operation is:
